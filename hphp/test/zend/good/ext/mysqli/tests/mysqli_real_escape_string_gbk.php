...
$port, $socket, mysqli_connect_errno(), mysqli_connect_error());
	...
	if (!mysqli_query($link, 'DROP TABLE IF EXISTS test_mysqli_real_escape_string_gbk_table_1')) {
		printf("Failed to drop old test_mysqli_real_escape_string_gbk_table_1 table: [%d] %s\n", mysqli_errno($link),
mysqli_error($link));
	}
...
mysqli_error($link));
	...
	var_dump(mysqli_query($link, "INSERT INTO test_mysqli_real_escape_string_gbk_table_1(id, label) VALUES (100, '��')"));
